...
* The **Hop Reliability Layer**, implemented on top of the transport layer.
* The **Hop** application, an SSH replacement, built on top of the reliability
  layer.
...
replacement. It uses the Hop reliability layer, and the SSH terminal management
layers. SSH is provided by the golang.org/x implementation. The client and
server are in the `hopclient` and `hopserver` directories.
