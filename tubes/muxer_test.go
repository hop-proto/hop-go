...
func TestOneMuxer(t *testing.T) {
	...
	go func() {
		...
		assert.Equal(t, byte(1), tube.GetID())
		assert.NilError(t, err)
		serverLogger.Infof("received tube 1")
	...
	assert.NilError(t, err)
	assert.Equal(t, byte(1), tube.GetID())
	clientLogger.Infof("created tube %d", tube.GetID())
	buf := make([]byte, len(s))
	n, err := tube.Read(buf)
	assert.NilError(t, err)
	assert.Equal(t, len(buf), n)
	assert.Equal(t, s, string(buf))
	clientLogger.Infof("received %q", string(buf))
	sWg.Done()
	wg.Wait()
