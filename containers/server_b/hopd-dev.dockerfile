...
COPY server_b/id_server.pem /etc/hopd/id_hop.pem
RUN chmod 600 /etc/hopd/id_hop.pem
COPY server_b/id_server.pub /etc/hopd/id_hop.pub
COPY server_b/id_server.cert /etc/hopd/id_hop.cert
COPY server_b/target_server_dev_config /etc/hopd/config
