...
def run_ssh_rsync(h1, file_size, bw):
    cmd = f'rsync --info=progress2 -e "ssh -c chacha20-poly1305@openssh.com" ./{file_size}_file root@10.0.3.10:/tmp/file/'
    ...
    with open(output_file, 'a') as file:
        for config in network_configs:
            ...
            for size in file_sizes:
                ...
                speed = run_ssh_rsync(h1, size, config["bw"])
                log_and_print(file, f"{datetime.now()};{file_size_mb};SSH Rsync;{config['bw']};{config['delay']};{config['jitter']};{config['loss']};{speed};")
                h3.cmd("rm -f /tmp/file/*")
