...
After receiving the Client Acknowledgment, the server decrypts the cookie using
a hash of the client ephemeral, IP address, and port as the associated data.
Successful decryption verifies that the server is talking to the same client
that initiated the Client Hello and locks in the client's
ephemeral public key. Successful decryption of the cookie also ensures that the
...
server.
At this point, both client and server... They still need to be
authenticated to
each other.
\TODO{}
