...
abstraction, but also provides the option for unreliable data delivery within a channel.
...
channel frame header. We borrow upon QUIC streams and statically allocate even
...
or server sends a Channel Initiation Request and the other party
replies with a Channel Initiation Response. Channels are closed by setting the
"FIN" bit on a channel frame. Multiple channels can be created and closed
...
Channel creation begins when one party (the initiator) sends a Channel
Initiation Request with Frame Counter set to 0.
%Unlike QUIC, 
