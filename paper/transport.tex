...
When the server receives the Client Hello, it performs the same logic as the
client. The server instantiates a new duplex object, absorbing the protocol name, message type, protocol version, and
...
AEAD encryption limit), and $H$ is a cryptographic hash (e.g., SHA-256).
...
After receiving the Client Acknowledgment, the server decrypts the cookie, which
verifies that the cliAt this point, both client and server... They still need to be authenticated to
each other.
\TODO{}
