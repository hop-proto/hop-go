...
protocol name (or version?). The protocol name, message type (client hello), protocol version, and the
...
client, absorbing the protocol name, message type, protocol version, and client
ephemeral public key into its own duplex object, and verifies that the MAC
squeezed out matches the MAC attached to the Client Hello. If so, the server
replies with a Server Hello, otherwise ignores the Client Hello.
