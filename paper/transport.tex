...
SSTP relies on a cryptographic handshake to set up an encrypted transport
tunnel between a client and a server. On a successful handshake, the client and
server will have established a shared session over which they can communicate
through encrypted channels, even in the case of loss of connectivity or IP
address migration.

SSTP supports two handshake modes: Discoverable Server and Hidden Server. In
Discoverable Server mode, an SSTP server will respond to all attempted
protocol compliant handshakes, revealing its presence as an SSTP server. In
hidden server mode, it will only respond to authenticated client requests, and otherwise will not respond.
...
\subsubsection{Discoverable Server}
...
\end{subfigure}
	...
	\textnormal{The discoverable server handshake
	takes 2.5~round-trips, while hidden server takes 1~round-trip but requires
	that the client be made known about the server out-of-band.}
	}
...
message fits into a single UDP packet. At this stage, both parties begin keeping
session state, but are not yet authenticated to each other.
...
The server static public key is delivered as part of the leaf certificate.
...
Upon receiving the Server Authentication message, the client absorbs the message type, reserved bytes, and Session ID, and
decrypts the certificates using the duplex material. It also calculates the DH result between the client ephemeral and the server static,
...
client (i.e., in the list of authorized keys or given access via an Authorization Grant (see Section~\ref{sec:authorization_grant})). If the
client is not authorized, the server drops the session and
...
\subsubsection{Hidden Server}
SSTP also supports a Hidden Server mode, which allows establishing connections with servers that
will not respond to any unauthenticated clients, including scans targeting the service. 
However, this requires that the client knows the server's static public key
prior to the handshake. This is useful for secure shell applications
where the client already knows the identity of the server. The Hidden Server
SSTP handshake will usually complete in a single round-trip
(Figure~\ref{fig:crypto_handshake_hidden}), barring any lost packets. We follow the Noise IK
handshake pattern~\cite{noise}.

\vspace{5pt} \noindent\textit{1) Client Hello+Authentication:}\quad 
Initially, the client knows the server's static public key
($s_s^{pub}$), acquired out-of-band. However, one of our design goals is that the server will not
reply to anyone who is not a valid client. Therefore, the first message in the handshake needs to
authenticate the client to the server. 

\input{bytefields/client_auth_hidden.tex}

The client instantiates a new duplex object and absorbs the protocol name, message type (0x7),
protocol version, reserved bytes, and client ephemeral public key. It absorbs the DH result between
the client ephemeral and server static, and uses the duplex material to encrypt its own static
public key. The Client Static Authentication tag is squeezed out. The client then absorbs the DH
result between the client static and server static, and encrypts a timestamp to prevent replay
attacks. A MAC is squeezed from the duplex object and appended to the Client Authentication
message (Figure~\ref{fig:client_auth_hidden}).

When the server receives the Client Authentication message, it instantiates its own duplex
object and absorbs the protocol name, message type, version, reserved bytes, and client ephemeral as
sent in the message. The server also absorbs the DH result between the client ephemeral and server
static, and uses the duplex material to decrypt the client's static public key. If the client static
public key is that of an authorized client, the server continues with the handshake. Otherwise, it
does not respond. It then absorbs the DH result between the client static and the server static,
and uses the duplex state to decrypt the timestamp. The server verifies that the timestamp is
greater than the last timestamp seen from the last valid handshake from the client. If the
verification fails, the server does not respond to the client.

\vspace{5pt} \noindent\textit{2) Server Hello+Authentication:}\quad 
Once the server has verified that the client initiating the handshake is an authorized client, the
server replies with a Server Authentication message. Continuing with its duplex object, it absorbs
the message type (0x8), reserved bytes, and a random 4~byte session ID. It generates a new ephemeral
key pair for this session and absorbs the DH result between the client ephemeral and server
ephemeral. Using the current state of the duplex, the server encrypts the leaf and intermediate
certificates. \todo{something about hidden servers should only be private servers, so will only host
a single service and only has 1 leaf certificate? } After encrypting the
certificates, it uses the duplex to squeeze out a MAC for the Server Authentication message
(Figure~\ref{fig:server_auth_hidden}).

\input{bytefields/server_auth_hidden.tex}

When the client receives the Server Authentication message, it performs the same operations such
that the duplex states are identical. It decrypts the certificates to check that they are valid and
verifies that the MAC squeezed from the full handshake duplex matches the MAC provided by the
server. 
...
the channel, the channel is closed and a new one is created. 
%The initial data field is ignored in the Channel Initiation request.
...
individual frames containing data or FIN messages. Channel handshake messages always have Frame Number
