...
SSTP also has a Hidden Server mode, which supports connection establishment with
servers that will not respond to scans targeting the service or any
unauthenticated clients. However, this requires that the client knows
the server's static public key. This is useful for secure shell applications
where the client already knows the identity of the server. The Hidden Server
SSTP handshake only takes a single round-trip
(Figure~\ref{fig:crypto_handshake_hidden}).

\vspace{5pt} \noindent\textit{1) Client Authentication:}\quad 
At first, the client already knows the server's static public key
($s_s^{pub}$),acquired out-of-band. However, one of our design goals is that the
server will not reply to anyone who is not a valid client. Therefore, the first
message in the handshake needs to authenticate the client to the server.


\vspace{5pt} \noindent\textit{2) Server Authentication:}\quad 
