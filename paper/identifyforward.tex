...
\subsubsection{Authorization Grant Channel}
Authorization Grant Channels are used by the principal, client, and server to run
...
Intent Requests and Intent Communications, the \texttt{Data} field is the \emph{Intent} (Figure~\ref{fig:intent}), which contains a hash of the ephemeral public key the client will
use as its temporary identifier, the name and port of the server it wishes to connect to, a single channel type, and any associated data for the channel
(e.g., a command to be run). The \texttt{Data} field in Intent Confirmation messages contain a
timestamp deadline set by the server for the client to complete the SSTP handshake, and
...
Once the principal approves the Intent Request, it communicates the Intent to the server.
...
from the server are forwarded back to the principal through the Network Proxy channel. Through this channel, the principal establishes an SSTP
...
we define this deadline or let it be set by the server?} The
