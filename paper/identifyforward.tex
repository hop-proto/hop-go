...
In this section, we present a protocol for secure identity forwarding. The purpose of delegation is
...
which allows a delegate to act under a principal's credentials only after the principal can verify
...
At a high level, the delegate requests that the principal grants permission for it to perform a
series of operations on the server, i.e., the \emph{intent}. If the principal approves the intent
...
server also approves the intent, the delegate can then establish an SSTP tunnel with the server
using the identity approved by the principal and can execute the specified commands.

\subsubsection{Intent Request}
Initially, the principal and the delegate have already established an SSTP tunnel between them.
...
contains the \emph{Intent Message} which is a serialization of an the following fields: the ID, address, and port of the destination
...
The principal needs to communicate the intent to the server in order to grant the
delegate permission to establish an SSTP tunnel with the server. The server will then also be able
to enforce that the delegate only executes the actions specified in the Intent Request. However, the
principal may not necessarily have connectivity to the server. If it approves the Intent Request,
...
will execute them. Note that
the communication between the principal and the server are through the SSTP tunnel, so any Intent
Communication messages
cannot be read or tampered by the delegate. 
...
delegate via the Delegation Channel from before. The delegate will then establish its own SSTP
...
principal and tear down any Delegation Channels created as part of the session.